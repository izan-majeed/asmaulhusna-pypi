...
`pip install asmaulhusna`

## This package provides 3 tuples and 2 functions.

```python
>>> from asmaulhusna import AsmaUlHusna
>>> x = AsmaUlHusna()
```
...
* The first fuction named as `display_nth_name(int)` takes an integer parameter which ranges from 1 to 99 and prints the nth name of Allah with corresponding meaning.

```python
>>> x.display_nth_name(25)
...
* The second fuction named as `get_nth_name(int)` takes an integer parameter which ranges from 1 to 99 and returns a list of nth name of Allah with corresponding meaning.

```python
>>> x.get_nth_name(13)
['Al-Musawwir', 'The Flawless Shaper', 'الْمُصَوِّرُ']
>>> 
```